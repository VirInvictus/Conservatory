...
**Version:** 0.7.0 (Phases 0–19 shipped, including Phase 9 (scrobbling, v0.3.1–v0.3.6), Phase 26 (de-adwaita, `0.3.0`), and the Phase 19 + 9 milestone (`v0.4.4`); v0.4.5 landed the Flatpak build-dependency modules, v0.5.0 the interaction tier, v0.6.0 the consumer wave, and v0.7.0 the data-safety stamp. See §17 and roadmap.md.)
...
> **Status note.** This is the design contract. The decisions below are settled enough to build against. As of v0.7.0 the app is a music library manager, a real player, a full podcast client, and an audiobook player: Phases 0–19 (§17) have shipped, Phase 26 replaced libadwaita with the owned stylesheet (`0.3.0`), the Phase 19 + 9 milestone shipped as `v0.4.4` (scrobbling, the waveform seek bar, drag-drop import, the full-screen Now Playing view, the local credits), v0.5.0 landed the interaction tier, v0.6.0 the consumer wave onto gtk4 0.11 and the vir-gtk widget kit, and v0.7.0 the data-safety stamp: the backup and restore verbs §9 always promised. The build is no longer deferred (the original deferral and its rationale are preserved in §16.1 and §17 for the record, since they are the thing to re-read if the concurrency with Atrium proves a mistake). Detail that was provisional at first draft (exact schema columns, CLI verbs, config keys) is now implemented; the `docs/` references carry the firmed-up specifics. Genuinely open decisions are collected in §16, not scattered as silent guesses.
...
- **`vir-search`** — the shared Calibre-shaped search expression language (lex / parse / AST / evaluator / SQL translator), typed against Conservatory's domain (Track / Album / Artist / Show / Episode / Book). It is a shared crate consumed as a branch-tracking git dep by both Atrium and Conservatory (the shared `vir-gtk` widget kit rides the same way) so the grammar and the widget tier cannot drift between the two apps. Deliberately feature-free: every field, podcast and audiobook included, is always compiled in; a field over an absent or empty table matches nothing, which keeps trait-object field registration out of the hot path.
...
Phase 26 drops libadwaita and keeps GTK4: the GNOME identity layer (adwaita stylesheet, adaptive widgets, preference rows, `adw::StyleManager`) is replaced by plain GTK4 widgets and a stylesheet Conservatory owns outright. The Colophon pilot (its spec, "Design language") is the template; the widget tree in §2.3 describes the adwaita shell and is rewritten as the migration lands. The decisions:

- **The look.** Flat, square, hard 1px borders, no shadows, denser spacing than the GNOME HIG. Kanagawa Dragon stays the one fixed palette, dark-only; the owned sheet bakes the Dragon hex values directly (no CSS custom properties: a single fixed palette gets nothing from them, and skipping them keeps the gtk4 crate on `v4_14` and CI on stock Ubuntu). The sheet registers one step above `STYLE_PROVIDER_PRIORITY_USER` so it outranks both the adwaita sheet during the migration and any `~/.config/gtk-4.0/gtk.css` skin after it (the Colophon discovery). Dark polarity is forced with `gtk-application-prefer-dark-theme`, replacing `adw::StyleManager` `ForceDark`.
- **Decoration posture.** Slim flat headerbar carrying the tab switcher, search, jobs, and the primary menu; **no window buttons** (`show-title-buttons = false`). Closing is `Ctrl+Q` plus the compositor's own binds on either desktop.
- **Tab switcher.** `GtkStackSwitcher` over `GtkStack`, text-only tabs (Music | Podcasts | Audiobooks; the adwaita switcher's icons are dropped), squared and flattened by the owned sheet. The narrow-width behaviour survives toolkit-agnostically: a manual width watcher hides the header switcher and reveals a bottom switcher bar at the same 550px threshold the `AdwBreakpoint` used, preserving the §2.3 bottom-bar stacking rule (Now-bar innermost, switcher bar beneath it).
- **Dialogs.** Owned modal `gtk::Window`s. the `Alert`/`Appearance` dialogs live in the shared `vir-gtk` widget kit since v0.6.0 (the local `ui/dialogs.rs` was deleted), mirroring the `adw::AlertDialog` surface (heading/body, extra child, named responses with suggested/destructive appearance, default and close responses, Escape and Enter behaviour) so call sites convert mechanically; stock `gtk::AlertDialog` was evaluated and rejected (no extra child, no per-response styling, index-addressed buttons). Preferences becomes a plain modal window with a `GtkStackSwitcher` over its pages; preference rows are owned `gtk::ListBoxRow` builders (`ui/rows.rs`); the shortcuts reference re-skins as a hand-rolled window (it was already a custom dialog by necessity).
- **Unchanged.** The Columns UI browse surface (already plain `gtk::Paned` + `gtk::ColumnView`), the bundled-font fontconfig mechanism, the per-album accent-ring provider, `config.toml` as the only settings store (no GSettings), and the GPL-3 posture. No feature regresses and the app keeps working under GNOME; only the look stops being GNOME's.
...
- **Equalizer**: a graphic EQ (stacked `equalizer` peaking bands at ISO centres) plus a parametric option on named biquad bands (`equalizer@p<k>`), with named presets. The obvious-looking `superequalizer` / `firequalizer` are avoided: they carry no runtime command, so every adjustment would rebuild the graph and gap the audio. `anequalizer` was rejected at build time: its live `change` command hangs the mpv command pipeline on the current FFmpeg (measured; the roadmap's 5.5b re-scope records it), so the parametric option is ordinary named bands (migration 0021), not the `af`-embedded graph this line once named.
...
```toml
[library]
root = "~/Conservatory"        # the default (§16.14); unset falls back to it
...
14. **Default library root (settled 2026-08-05).** The config default is **`~/Conservatory/`**: an owned directory at the home root, symmetric `{Music,Podcasts,Audiobooks}/` subtrees, no stutter. The precedent is Calibre itself, the app's stated model: an application that owns and moves a library gets its own directory (`~/Calibre Library`), it does not nest inside an XDG media dir. The §5.1 `Music/` layout stands unchanged beneath it. A CLI positional still overrides (dev / tooling), and the Preferences folder-picker (Phase 10a) re-points it freely; the default only decides what an unconfigured launch sees. Alternatives considered and rejected, recorded in case this needs revisiting: `~/Music/Conservatory/` keeps everything under `~/Music` but reads `~/Music/Conservatory/Music/`, a triple-`Music` stutter Brandon dislikes; rooting at `~/Music` itself with the `Music/` prefix dropped for the music tree is stutter-free but asymmetric, and it turns the standard XDG music dir into an app-managed tree where any stray file is foreign.
...
- **Phase 9.** Listening history: optional ListenBrainz / Last.fm scrobbling, off by default. **Done (v0.3.1–v0.3.6).**
...
The manager half (Phases 1–3) must be usable before the player half is finished, and the player must be usable before podcasts arrive. Audiobooks (Phase 7) complete the three media types; the maintenance, configuration, and polish phases (8, 10–17) build on that base. Phases 1–17 are done plus Phase 26 (pulled forward to `0.3.0`); Phases 18–20 are the runway to `1.0.0`, and Phases 21–25 are the committed post-1.0 tiers. Each is a hard phase that leaves a usable artifact. No phase leaves the app non-functional.
...
- `data/` — icons, the `.desktop` entry, the AppStream metainfo template, the Flatpak manifest, the vendored cargo sources, bundled fonts (registered via fontconfig at first run; never assume host fonts).
- `conservatory-core/`, `conservatory-podcasts/`, `conservatory-audiobooks/`, `conservatory-cli/`, `conservatory/` — workspace members (`conservatory-podcasts` and `conservatory-audiobooks` are the compile-time plugin crates, §2.2).
