...
| `2.1.0` | Hyprland-leaning design | 25 | committed, beyond 1.0 |
| `2.2.0` | Hyprland-native design (de-adwaita) | 26 | committed, beyond 1.0 |
...
### Phase 25 — Hyprland-leaning design (`2.1.0`)

Brandon's desktop moved from GNOME Shell to Hyprland (a Wayland tiling compositor); this phase audits and hardens the app for a tiling, keyboard-first, GNOME-shell-free environment. **Strictly additive**: nothing here may regress the GNOME/default-desktop experience (the CLAUDE.md "never break userspace" rule); every item is either a narrow-width fix that also helps a small floating GNOME window, a new opt-in surface, or a verification pass with no code change implied unless a gap is actually found.

Direction note (Brandon, 2026-07-09): the portfolio goal has since moved past "runs politely under Hyprland" to "fully belongs on Hyprland," which means dropping libadwaita while keeping GTK4; that lands as Phase 26, gated on the Colophon pilot. This phase's audit items stay valid (toolkit-agnostic geometry, keyboard, portal, and doc work), with one execution consequence: where an item below offers a choice between adopting more adwaita (e.g. converting the tab bodies to `AdwNavigationSplitView` per the spec §2.3 diagram) and a toolkit-agnostic fix (a manual narrow-width collapse), prefer the toolkit-agnostic option so Phase 26 has less to unwind.
...
*Usable artifact:* the window tiles cleanly at half/third/quarter-monitor Hyprland geometries without clipped panes, a compact Now Playing mode exists that a `windowrulev2` rule can float onto a scratchpad, the app is confirmed to run with full functionality and no silent degradation outside a GNOME session, and `docs/hyprland.md` captures the setup. The GNOME/default-desktop experience is unchanged pixel-for-pixel.

### Phase 26 — Hyprland-native design: de-adwaita (`2.2.0`)

Portfolio direction change (Brandon, 2026-07-09): the goal moved from "runs politely under Hyprland" (Phase 25's frame) to "fully belongs on Hyprland." Concretely: drop libadwaita, keep GTK4. GTK4 is Wayland-native and stays; what goes is the GNOME identity layer (the adwaita stylesheet, the adaptive widgets, the preference-row family, `adw::StyleManager`), replaced by plain GTK4 widgets and an application stylesheet Conservatory owns outright, styled flat and tiling-first around the Columns UI browse surface (which is already plain `gtk::Paned`s and `gtk::ColumnView`, the least adwaita-shaped UI in the portfolio). Colophon pilots the move (its roadmap, Phase 6); Conservatory follows once the pilot's patterns (widget replacements, generated owned stylesheet) are proven. "Never break userspace" binds in full: no feature regression, the app keeps working under GNOME, and the spec §13 memory budgets hold (a widget-and-CSS migration should be memory-neutral; anything that escalates gets its own `heaptrack`/`massif` checkpoint, per the Continuous rules).

- [ ] **Go/no-go and sequencing against the pilot.** Review what Colophon's Phase 6 proved and cost, then inventory Conservatory's full `adw::` surface with a mapped plain-GTK replacement per type: the `AdwViewStack`/`AdwViewSwitcher` three-tab header, the `AdwToolbarView` root, the `adw::AlertDialog` + preference-row set (bulk edit, per-show settings, `SpinRow`/`SwitchRow`/`ComboRow`), the Preferences window, the `adw::PreferencesDialog`-based shortcuts reference, toasts, and the `AdwStyleManager` `ForceDark` call (`main.rs:200`). Conservatory is well positioned: no GSettings (config.toml), fonts bundled, dark-only Kanagawa already forced rather than portal-followed.
- [ ] **Design decisions land in spec first.** Decoration posture (header cargo: the tab switcher, search, primary menu; whether window buttons render at all), the tab switcher's plain-GTK successor (`GtkStack` + `GtkStackSwitcher` or a custom flat switcher), the dialog set's successors, and the shortcuts reference rebuilt hand-rolled (it is already a custom dialog by necessity; it just re-skins). Dark/light is the easy case here: the app is unconditionally dark-Kanagawa, so no portal work is needed, only the removal of the `ForceDark` adwaita call.
- [ ] **The owned stylesheet.** Author the application sheet (flat, square, hard borders, denser spacing; the Dragon palette mapping in `docs/theme.md` retargets from libadwaita named colours to owned classes). Reuse the pilot's generated-sheet machinery where it transfers.
- [ ] **Packaging and docs follow-through.** Evaluate the Flatpak runtime move (GNOME → freedesktop) once libadwaita is gone; update spec §2.3's widget-tree diagram and `docs/theme.md` to the new posture; `docs/hyprland.md` (Phase 25) gains whatever setup notes change.
- [ ] **Verification tail.** Phase 25's geometry/keyboard/session-independence audit items re-run against the migrated shell as this phase's acceptance criteria, plus a `mem_check` pass to confirm memory neutrality.

*Usable artifact:* Conservatory runs the same feature set with zero `adw::` symbols in the workspace, its own flat tiling-first stylesheet on all three tabs, and no regression under GNOME; the memory budgets and the Phase 25 usable artifact both still hold.
