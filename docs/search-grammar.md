...
| `show:` / `is:in_inbox` / `pub:` | podcast | as Belfry §3.7 |
...
- **Date keywords:** `today`, `yesterday`, `tomorrow`, `thisweek`, `lastweek`, `nextweek`, `thismonth`, `lastmonth`, `nextmonth`, `thisyear`, `Ndaysago`, `in<N>days`, the compact `+Nd` (forward) / `-Nd` (back) offsets, plus `YYYY`, `YYYY-MM`, `YYYY-MM-DD` with field-count precision (the full vir-search keyword table; attribution corrected 2026-09-15: `tomorrow`/`lastweek`/`nextweek` shipped in vir-search 1.0.4 and `lastmonth`/`nextmonth` in 1.4.1, not all in 1.3.0 as this list claimed, and the relative-offset forms were missing until then).
