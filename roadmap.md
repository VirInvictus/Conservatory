...
Brandon's desktop moved from GNOME Shell to Hyprland (a Wayland tiling compositor); the app stays GTK4 / libadwaita, CSD, and default-dark-Kanagawa exactly as it is, but this phase audits and hardens it for a tiling, keyboard-first, GNOME-shell-free environment. **Strictly additive**: nothing here may regress the GNOME/default-desktop experience (the CLAUDE.md "never break userspace" rule); every item is either a narrow-width fix that also helps a small floating GNOME window, a new opt-in surface, or a verification pass with no code change implied unless a gap is actually found.
