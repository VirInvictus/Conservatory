...
- [x] (Later → shipped 2026-09-04) the parametric option, re-scoped after audio measurement (see below): **parametric bands as named `equalizer@p<k>` biquads inside the `@eq` stage, not `anequalizer`.** User-defined bands (arbitrary centre frequency, Q, gain) persisted in their own table (migration, the 0008 precedent), rendered into the `@eq` stage after the graphic bands, applied at load; per-band *gain* edits ride the shipped live `af-command` gain path; *frequency/Q* edits are structural rebuilds (the flat↔non-flat precedent — rare edits, and the live commands for them are not trustworthy, see the finding). CLI verbs (`peq list/set/clear`), engine `SetPeq`, persistence, tests; the GTK editor is explicitly out of this box's scope.
  ...
  *(Implemented 2026-09-04: migration `0021` (`peq_bands`), `PeqBand` model with
  range validation in the replace-all write, the extended `eq_stage` /
  `build_af_chain`, host `set_peq` + `set_peq_band_gain`, engine
  `SetPeqBands` / `SetPeqBandGain`, and the CLI `peq show|set|remove|clear`
  verbs; `play` pushes the persisted set at startup, as it does the graphic EQ.
  The GTK parametric editor stays out of this box by design; the shipped live
  gain path covers per-band gain, and frequency/Q edits rebuild structurally.
  Tests: chain builder units, a worker round-trip + atomic-validation test, and
  an engine null-host integration that rebuilds and live-edits mid-playback to
  EOF.)*
...
- [x] Tests: shelf/filter model logic; book-state derivation; Perspective save/reload over books. *(Verified 2026-09-06: the first two clauses shipped with the work they name, `book_query::filter_books` carries its own test mod (`book_query.rs:72+`) and `BookState::derive` is covered (`conservatory-core/tests/audiobooks.rs:425-431`); the third clause was written in error, Perspectives are music-only and never had a book surface, so there was nothing to test. Struck, not built.)*
...
- [x] (Minor) Rating normalization across player conventions on read (POPM scale differences between WMP, foobar2000, and DeaDBeeF), the Lattice `tags.py` / `rerate.py` lesson, so imported ratings land consistently on the 0 to 5 scale. *(Shipped 2026-09-06: the WMP read buckets and the foobar2000/WMP/MusicBee canonical bytes were already right; the gap was DeaDBeeF, which writes ~stars*63.75 (2*→127, 3*→190, 4*→254) and read one star high through the buckets. Its odd bytes, which no canonical writer produces, are now exact-matched to the intended stars ahead of the buckets; the 64 byte is the one DeaDBeeF-1*/canonical-2* collision and keeps the canonical reading. `stars_from_popm_byte` + a dedicated test.)*
...
- [x] **Deferred at 9b, shipped at 9d (v0.3.6).** The ephemeral "now playing" update on load. It bypasses the outbox (a live ping to the service, a different transport than the completed-play queue) and is not needed to scrobble, so it landed as the small follow-on this line predicted rather than bloating the completed-play path. Phase 9d shipped it on **both** services alongside the real submission rule (30s floor, half-or-four-minutes, stamped with the play's start time). Ticked in the 2026-07-23 reconciliation sweep.
...
*Usable artifact:* Last.fm works as an alternative to ListenBrainz. **Tags `0.4.4+`** once Phase 19 also lands (the milestone label was re-anchored 2026-09-05; see the version table).
...
*Usable artifact:* scrobbling behaves like a first-class scrobbler: a now-playing indicator on the service, partial plays past the threshold counted, short tracks and near-start skips not. Part of the `0.4.4+` tier.
...
- [x] `Rating`/`Added` facets (the easy follow-ons). *(Shipped 2026-09-06: `FacetField::Rating` shows `Unrated` plus the 1-5 star counts, and `FacetField::Added` buckets `added_at` to its added month (the grammar's `added:YYYY-MM` precision, so ISO order is chronological). Both are single-valued partition facets that narrow downstream panes like any other; they sit at the end of the pane menu, so existing `[browse].panes` configs are untouched. Fixture tests: the partition set covers both, plus the worded unrated bucket and the backdated-month narrowing.)*
- [ ] **Deferred:** live pane rebuild on prefs close; `Composer`/`Work` facets (need new schema).
...
- [ ] **Idle memory at 50k:** confirm the spec §13 idle target (< 200 MB) on a real ~50k-track library, since the synthetic fixture tops out at 12k and the 12k idle (~196 MB) extrapolates to ~215–230 MB at 50k. If over, optimize before the `v1.0.0` tag. *(One gate, two boxes: the Phase 20 section carries the copy; one session ticks both.)*
- [ ] **Full-library move-safety pass:** run the 15a move/undo/crash checks against a working copy of Brandon's real library (the synthetic-only pass covered 0.1.0). *(One gate, two boxes: the Phase 20 section carries the copy; one session ticks both.)*
...
*(Re-anchored 2026-09-05: the `0.4.0` number was consumed by the untagged 0.4.0-0.4.3 point releases; see the version table.)*
...
size were gating the milestone tag as a single line, and the vaguest one would
...
### Bugs to Fix
- [x] **Author Shelf Sorting:** Sort audiobook authors by their parsed `sort_name` (last-name-first) rather than `author_display` (given name). *(Fixed 2026-09-04: `BookListRow` denormalizes the authors' `sort_name` join and the Author key sorts on it; regression pins a pair whose display and sort orders disagree.)*
- [x] **Multi-Disc Path Splitting:** Fix `apply_db_path` dynamically walking up a single `parent()` to correctly locate the album root, preventing covers from saving into disc subfolders. *(Fixed 2026-09-04: the album folder is now the deepest common directory of the job's planned destinations, read from the journal inside the completing transaction; order-independent, roll-forward-stable, undo folds the from sides.)*
- [x] **Journaled Cover Moves:** Move `follow_cover` inside the `mover::apply` transaction so that cover art moves are properly recorded in the undo journal. *(Done 2026-09-04: the book cover is its own journaled `MoveOp`; `apply_db_path`'s book branch rewrites `books.cover_path` under a `(book_id, from)` guard, the chapter-rewrite pattern, so the file, the pointer, and undo all move together.)*
- [x] **Missing Accent Updates:** Pass the computed `accent_rgb` to `set_album_cover_path` during track imports into pre-existing albums. *(Fixed 2026-09-04: the post-move cover write passes the plan-time accent, so a matched album's accent refreshes; regression seeds a sentinel and requires the re-import to replace it.)*
...
- [x] **Docs Sync:** Correct `README.md` workspace version, remove phantom `conservatory-search` references, and document actual CLI verbs (`import`, `organize`, `shelf-genre-set`). *(Done 2026-09-04: version + crate list in the 0.4.3 docs repair; the quick start now shows `shelf-genre-set` → `organize --apply` with their real argument shapes.)*
...
- ~~**Milestone gate (the next tag, `0.4.4+`).**~~ *(Closed 2026-09-06: `v0.4.4` tagged on Brandon's go with 19b-iii shipped. The 19b-i display pass and the audible EQ re-check are still owed as hands-on follow-ups; anything they surface lands as a `0.4.x` fix.)*
...
## Reconciliation addendum (2026-09-06, the audit's floating-deferral finding)

- [ ] **SQL push-down for the wildcard and list match kinds.** The Unreleased
  patchnotes bullet for the vir-search 1.4.0 adoption says the new kinds'
  SQL push-down is "deferred and tracked", but no box tracked it; this is
  that box. `Prefix` (`foo*`), `Suffix` (`*bar`), and `In` (`(a,b)`) have
...
### Final audit 2026-09-13 (THE FINAL AUDIT: NEW findings, one line each; full detail in audit-final/Conservatory/FINAL-REPORT.md)
- [ ] MED — backup.rs:74-77 `restore` deletes the live -wal/-shm BEFORE the rename commit point: a crash in the window strips WAL-resident commits (contradicts the module's own docstring). Rename first, remove replaced sidecars after.
- [ ] MED — Move-preview dialog says conflicts "will be skipped" but mover::apply refuses the whole job on any conflict, and run_scoped_move rebuilds ops from all rows (window.rs:4222, mover/mod.rs:205-211). Fix wording or filter conflicted ops.
- [ ] MED — Music import inserts rows before mover::apply with no MissingSource pre-check (import/mod.rs:179-305); the "database untouched" contract holds only for pre-check-visible conflicts. Audiobook twin already recorded (partial-commit).
- [ ] MED — Podcast feed bodies buffered unbounded (fetcher.rs:171): stream with a size cap (spec §13 budget).
- [ ] MED — Version carriers regressed: README badge/status/phase-list and spec masthead still say v0.5.0; tree is 0.7.0; add the masthead to the release-checklist carrier set.
- [ ] MED — "Nightly DB backup and the JSON export" (spec.md:468, schema.md:14) describes mechanisms that do not exist: verbs are manual-only and no curated-JSON export exists. Reword or land the backup-rotation feature.
- [ ] MED — spec §9 CLI synopsis stale on six lines (import --dry-run, organize --dry-run, shelf-genre set, queue reorder, play "standalone mpv", --tsv/--json/--human flags); §7.5 audiobook metadata precedence order wrong (sidecar > embedded > folder in code); ID3-CHAP + `id3` dep claimed in three docs but absent from code; symphonia still described as live (lofty signed off over it at Phase 1c); spec.md:199 still teaches `is:finished false`; mangled `{series_index2}` token at spec.md:484.
- [ ] MED — GPL/ffmpeg filter lists disagree across spec.md:653, ATTRIBUTIONS.md:70, docs/libmpv-profiles.md:48/:55 (rubberband/anequalizer stale; as-built set: silenceremove, acompressor, equalizer, alimiter, dynaudnorm, volume).
- [ ] MED — "Six crates" miscount in README.md:148, spec.md:94, CLAUDE.md:35: five workspace members; vir-search/vir-gtk are git deps, vir-gtk unmentioned.
- [ ] MED — Comment truth pass (one commit): audiobooks.rs header denies shipped playback; engine.rs save_cursor doc contradicts its body; tags.rs APE-stripping caveat stale (ape strip shipped); ape.rs header wrong about helper location + nonexistent strip_file; phantom conservatory-search in query.rs and reads.rs comments; podcasts/lib.rs broken credentials rustdoc link; stacked superseded re-export comments in ui/mod.rs; six Phase-1-era future-tense module headers.
- [ ] MED — GitHub: v0.7.0 has NO Release (Latest advertises v0.6.0; only v0.6.0 has one at all) — execute decision 60's Releases slice + backfill; add tag-triggered release.yml (tag message is already the verbatim patchnotes entry); CI never runs the meson appstream/desktop validators.
- [ ] LOW — Docs staleness tail: scrobble.md pre-9d header/verb list; import.md + mover.md "out of scope" lists naming shipped work; roadmap milestone table missing 0.4.5/0.5.0/0.6.0/0.7.0 rows; 0.2.0 "tagged" claim; path-template examples missing Music/ prefix; GNOME-era .ui/GSettings remnants in spec; spec.md:145 cites deleted ui/dialogs.rs; ATTRIBUTIONS credits conservatory-search; CLAUDE.md:3 points at the retired ~/.claude path; patchnotes v0.5.0 duplicate bullet + v0.6.0 open-work parenthetical.
- [ ] LOW — Em-dash house-rule violations: 262 instances (roadmap 181, of which 159 are `## Phase N —` heading separators; mechanical colon recast kills ~180).
- [ ] LOW — CLI help main.rs:549-550 says whole-tree audiobook batch import is "a later phase" (shipped v0.4.4); scan.rs walker has no symlink-cycle guard; episode folder_path recomputed from current feed title diverges from audio_path (retitle orphans files); 38 `let _ = rt.block_on(worker.*)` sites swallow DB-write failures silently.
- [ ] LOW — Comment LOWs: shelf_genre UNKNOWN knob write-only while the GUI row promises next-launch effect (unwired preference, fix comment + wire or remove); PlaylistOrder "random deferred to Phase 17" stale; tags.rs title says reader-only; search mod/domain headers missing; eval/sql_translate cite "atrium-search"; ui/podcasts.rs sub-millisecond parenthetical pre-dates VACUUM backup on the worker.
- [ ] LOW — Housekeeping: delete tests/.gitkeep + empty root tests/, data/.gitkeep, docs/.gitkeep; stale GSettings .gitignore block; add /.flatpak-builder/ to .gitignore; Cargo.toml repository URL lowercase vs canonical; .claude/ local debris (machine-ignored only); IBMPlexMono-Medium.ttf is bundled but never selected (trim decision inside Phase 20 payload review).
- [ ] LOW — GitHub hardening: setup-uv@v5 deprecation + dead cache glob (fix Atrium's mirror job in the same lane); SHA-pin actions; concurrency group; actions-only dependabot; private vulnerability reporting disabled + no SECURITY.md/CONTRIBUTING/templates (docs-sweep tail).
- [ ] Feature candidates logged (FINAL-REPORT L4, ranked): nightly/rotating backup automation; missing-file audit tier + MISSING verify verdict; curated-layer JSON export; waveform envelope for downloaded episodes; MPRIS Shuffle/LoopStatus; Show-in-Files action; watched import folder (dependency-flagged, dep-free inbox rescan first). Re-ranked: Phase 20 packaging #1; Viaduct mem_check harness BEFORE the 50k gate.
