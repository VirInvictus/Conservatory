...
- [ ] **8c-iii strip:** The **fix** (a byte-level APE strip, the `apestrip.py` technique) lands as a mutating `apestrip` verb: dry-run by default, crash-safe temp+fsync+atomic-rename writes, and an exact undo (the excised bytes are journaled in `ape_strips` before the file is touched). lofty cannot strip APE on MPEG (the Phase 5b deferral); it is byte surgery, not a lofty call. The detect-and-fix split mirrors duplicates (8b) reporting then a fix verb doing the cleanup. (Optional APE→ID3 migration deferred to a follow-up; the DB is source of truth and `embed-tags` rewrites canonical ID3.)
