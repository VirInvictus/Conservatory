...
- **Gapless:** `--gapless-audio=weak` (preserves source rate on a mixed-rate library; `audio-samplerate` / `audio-format` left unset to avoid needless resampling). **The property alone is not enough** (learned at v0.1.22): reacting to EOF with a replace-mode `loadfile` tears the decoder down, so mpv never gets to decode across the boundary and every transition audibly gapped. The engine therefore **prefetches**: when the coming boundary is track→track with gapless profiles and no stop-at-boundary armed (`should_prefetch`, pure), it appends the next queue item to mpv's internal playlist (`loadfile … append`); mpv crosses the boundary itself, and at the EOF event the engine advances its bookkeeping and applies the new track's chain via `apply_profile` (no reload) — the `@rg` head swaps at the event, so a large per-track gain step can bleed for the ~buffered fraction of a second (album gain within one album is identical, so the common case is artifact-free). Every queue mutation and stop-after-current / end-of-item-sleep toggle resyncs the appended entry (`playlist-clear` + re-append); spoken word never prefetches (sessions, resume seeks, and per-show chains want the explicit load path).
...
- **Realized chain (Phase 6c-i, `player/spoken.rs`):** the stages append after the music stages of `build_af_chain`, Smart Speed before Voice Boost. Smart Speed = `@ss:lavfi=[silenceremove=stop_periods=-1:stop_duration=<d>:stop_threshold=<t>:stop_silence=<s>]` (mid-stream removal, leaving a short beat), the gate set by the global **level** (v0.1.2, `audio_state.smart_speed_level`): **Gentle** `d=0.5, t=-30dB, s=0.3` (default, the v0.1.1 retune), **Balanced** `0.35 / -30dB / 0.2`, **Aggressive** `0.3 / -28dB / 0.15`. The original `-40dB / 1s` was measured (ffmpeg over real episodes) to remove *nothing*: mastered-podcast "silence" is room tone / breaths / music beds around -25 to -30 dB, not below -40 dB, and pauses are shorter than 1 s. Voice Boost = three labelled stages: `@vbcomp` (`acompressor=threshold=<-24 dB linear>:ratio=4:attack=5:release=150:makeup=2`), `@vbeq` (`highpass=f=80,equalizer=f=2500:t=o:w=1:g=3`, a low-cut + presence lift), `@vbnorm` (`dynaudnorm=g=15:p=0.9`, a tighter window than the music leveler's `g=31`). Voice Boost's params are fixed; the Smart Speed gate is whichever of the three global levels is set (the per-show / per-book setting is only Smart Speed's on/off, not the aggressiveness). A show with no saved settings resolves both flags off (configured-shows-only). The level is an af-chain parameter, so it lives on `audio_state` next to the DSP settings and is applied live via the `set_dsp` path (`MpvHost::set_smart_speed_level`). Time-saved accounting is 6c-ii: `smart_speed_saved = max(0, audio_seconds/speed − real_seconds)`.
