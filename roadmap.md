...
- [ ] **App-id decision (blocks the item below, Brandon's call).** Three spellings are in play and the docs contradict the code. `spec.md` §12 and §15 offer `org.gnome.Conservatory` (if GNOME Circle accepts) or `io.github.virinvictus.Conservatory`; the code has shipped `org.virinvictus.Conservatory` since early on (`conservatory/src/main.rs:22`), and the Phase 25 mini-player item below already calls that id "already fixed" and treats it as the stable anchor for a Hyprland `windowrulev2`. It is load-bearing and hard to reverse: it names the Flatpak, the `.desktop` basename, the metainfo id, the GSettings path, and any window rule keyed on it. Settle it, then sync `spec.md` §12 / §15 and this line to the answer. *(Surfaced by the 2026-07-23 reconciliation sweep; the sweep deliberately did not pick a winner.)*
- [ ] **Flatpak + AppStream:** the manifest under whichever app id the item above settles on, a validating `metainfo.xml` (releases tag, 16:9 screenshots, SPDX license, `appstreamcli validate` clean), the Meson packaging wrapper wired end-to-end, and GNOME Circle readiness (§12). The final icon pass (a conservatory silhouette, §15).
