...
- [ ] **26n (`0.3.0` tag): verification tail + release.** `docs/hyprland.md` authored; CI green; both binaries smoke-run live. Remaining: Brandon's hands-on acceptance pass (see the Verification-tail item above), then the patchnotes milestone entry, VERSION + Cargo.toml to `0.3.0`, tag.
...
- [ ] **Verification tail.** Verified so far (2026-07-11): all CI gates green including the music-only lane (repaired at 26k1 after being red since v0.2.2); `cargo tree` adwaita-free under both feature sets; both binaries smoke-launch and run under the live session with no errors (portal warnings only, expected without `xdg-desktop-portal`, noted in the new `docs/hyprland.md`, authored). **Remaining, Brandon's hands-on acceptance:** the visual eyeball of the flipped look on Hyprland and GNOME (every tab, dialogs, Preferences, toast, narrow collapse at 550px, no-window-buttons posture, and with a themed `~/.config/gtk-4.0/gtk.css` present); the 26i audible A/B (EQ band drag, compressor toggle, `--debug` af rebuilds, `audio_state` readback + persistence reopen); the folded Phase 25 audits (tiling geometry floor at half/third/quarter tiles, small-tile dialog sizing, mouse-free keyboard walk + seek-step decision, no-GNOME session checks: keyring degradation, portals, logind inhibitor, MPRIS via playerctl/waybar, spectrum at fractional scale); and the memory pass (`VmHWM` vs the `v0.2.0` tag on the real library). The `0.3.0` tag waits on this pass.
