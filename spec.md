...
7. **ReplayGain scan vs read (settled, Phase 5c).** Conservatory **scans in-app**, it does not only read. A ReplayGain 2.0 scanner (modeled on Lattice's `scripts/replaygain.py`) computes album + track gain/peak for untagged or partially-tagged albums and writes the tags, refreshing the DB `replaygain_*` columns so the playback profile resolution (§6.2) sees them. The read-only path (Phase 4a) stands as the default; the scan is opt-in maintenance (Phase 5c). The one remaining sign-off is the scan mechanism: `rsgain` over libebur128 (the Lattice technique, a new external tool) versus driving ffmpeg's `ebur128` filter directly (no new runtime dependency, since libmpv already links ffmpeg).
