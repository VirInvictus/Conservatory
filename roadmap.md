...
- [x] MED — backup.rs restore WAL ordering. Fixed 2026-09-15 as checkpoint-then-rename-then-sweep (the busy database refuses): the WAL is folded into the main file through SQLite's own crash-safe path before the commit point, so neither suggested ordering's window exists. Three regression tests pin the windows.
- [x] MED — Move-preview vs apply. Filter approach 2026-09-15: run_scoped_move hands apply the plan's actionable set (apply's refusal stays as the backstop), and the flow moved off the GTK thread. Integration test pins plan-filter-apply end to end.
- [x] MED — Import pre-commit integrity pair. 2026-09-15: the music pre-check is now the mover's own plan over the provisional ops (MissingSource included, sidecars covered), and the audiobook importer heals interrupted jobs before the pre-check instead of after the rows. Regression tests for the vanished-file path and the doomed-recovery refusal.
- [x] MED — Podcast feed bodies stream through a 32 MiB cap; over-cap is FetchError::BodyTooLarge. (2026-09-15)
- [x] MED — Version carriers moved to 0.7.0 (badge, status line, shipped-phase list, spec masthead and status-note anchor; "daily driver" softened per the recorded call); the masthead is now in the carrier set named in CLAUDE.md.
- [x] MED — Nightly fiction reworded to the as-built truth (Brandon's gate answer: reword now, slot the feature); backup rotation and the curated-JSON export are ordered boxes in the slotted-features section below.
- [x] MED — All of it: §9 rewritten from the clap definitions, §7.5 reordered to the code's sidecar > embedded > folder, ID3-CHAP + `id3` struck (and the chapter sources slotted below), symphonia firmed to considered-and-not-adopted, `NOT is:finished` taught, the `{series_index:02}` token fixed.
- [x] MED — Filter lists reconciled on the as-built set across spec 15, ATTRIBUTIONS, and libmpv-profiles (highpass named in the spoken-word chain; anequalizer stays measured-and-rejected).
- [x] MED — Crate recount landed: five workspace crates plus the vir-search/vir-gtk git deps, in all three docs.
- [x] MED — Comment truth pass landed as one commit, all sixteen sites.
- [x] MED — All three: v0.7.0 Release cut plus the v0.5.0/v0.4.5/v0.4.4 backfill (Latest = v0.7.0), release.yml mints future Releases from the tag message behind an exists-check, and the fmt job runs the meson validators. (2026-09-15)
- [x] LOW — Docs staleness tail: everything as-built except the two patchnotes nits, which are recorded-not-edited (past entries are cut into tags and Releases; editing them would desync the file from the record).
- [x] LOW — The mechanical recast landed: 159 roadmap heading separators plus 13 ATTRIBUTIONS separators to colons. The prose tail (~90 across spec/patchnotes/docs) stays a recorded finding, out of lane scope.
- [x] LOW — CLI help fixed; the silent block_on sites (46 counting the split-line twins) now route through ui::log_worker_err. The symlink-cycle guard and the episode folder_path freeze are slotted below (real work, not blitz edits).
- [x] LOW — All the comment LOWs fixed; wiring the default_unknown preference is slotted below (the audit's fix-the-comment half is done; the wire-or-remove half needs a go).
- [x] LOW — Housekeeping executed per Brandon's gate answers; IBMPlexMono-Medium stays for the Phase 20 payload review.
...
Each box needs its own go before it is built; the mem_check harness lands
...
- [ ] Library-health GUI page (the threading split it needed landed 2026-09-15) and listening-history surfaces (the sessions table has been written since 6c-ii and is read by exactly one CLI printer).
- [ ] MPRIS Shuffle + LoopStatus parity: the engine has both states and the zbus wiring exists for the rest, but headset and desktop controls cannot see them today.
- [ ] Waveform envelope for downloaded episodes (waveform.rs is format-agnostic; audiobooks stay a recorded open question).
- [ ] Show-in-Files context action (`gtk::FileLauncher`; the Calibre idiom).
- [ ] PEQ band editor (gated on Brandon's audible re-check).
- [ ] Podcast chapter sources beyond `podcast:chapters` JSON (podlove PSC, ID3-CHAP): the Belfry sources that were not ported; the spec's chapter claim was struck to match the code on 2026-09-15.
- [ ] Wire the `genre.default_unknown` preference (currently write-only: the resolver hardcodes "Unknown"; found in the comment sweep).
- [ ] Episode `folder_path` freeze: refresh recomputes it from the current feed title while `audio_path` keeps the download-time folder, so a retitled episode orphans its file on re-download; freeze `folder_path` once `audio_path` is set.
- [ ] scan.rs walker symlink-cycle guard (a directory loop recurses toward PATH_MAX; no cycle guard today).
- [ ] Watched import folder: **DEPENDENCY FLAG** (the `notify` crate needs spec §11 sign-off before any build); the dep-free startup inbox rescan needs no sign-off and sequences first. Slot only.
