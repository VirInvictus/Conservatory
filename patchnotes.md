...
- **Fixed:** `conservatory-cli restore` no longer opens a crash window that silently discards WAL-resident commits in the module that exists for the data-safety contract. The live database's write-ahead log is checkpointed (TRUNCATE) through SQLite's own crash-safe path before anything is touched, the backup is copied through the same-directory temp file into an atomic rename, and the emptied sidecars are swept after the commit point; a busy database (a live reader) refuses instead of proceeding. Every intermediate state is safe: a crash before the rename loses nothing, and a crash after it leaves an inert empty `-wal` beside the restored file, re-initialized rather than replayed on the next open. Three regression tests pin the windows.
- **Fixed:** the move preview's "conflicts will be skipped" promise is now the behavior. `run_scoped_move` used to hand `mover::apply` the raw op list, and apply refuses a job with any conflict, so one conflicted file killed the whole scoped move the dialog had promised to work around. The plan's actionable set is what moves now (apply's refusal stays as the race backstop), a toast reports the moved and skipped counts, and the integration suite pins the plan-filter-apply contract end to end.
- **Fixed:** the importers' pre-commit integrity pair. The music import's hand-rolled pre-check saw only duplicate and existing targets, so a source vanishing between the scan and the persist pass slipped through, failed apply's own re-plan, and left the freshly written rows behind a reported failure; the pre-check is now the mover's own plan over the provisional move list (audio plus claimed sidecars), so every apply-visible conflict refuses with the database untouched. The audiobook twin's partial-commit window closes structurally: recovery heals interrupted jobs before the pre-check, not after the rows are written, so a recovery failure can no longer strand a persisted book.
- **Fixed:** the threading lane. The scoped move plus its cover resync and the audiobook shelf re-shelve no longer `block_on` the GTK main thread for their whole duration (a big batch could freeze the window for minutes): they ride `rt.spawn` with a `glib::spawn_future_local` completion, the window's drop-import bridge idiom. The mover's per-file `relocate` / `revert` run on the blocking thread pool instead of the GUI's one-worker runtime, so a large job no longer starves the player, MPRIS, and the scrobble outbox while it runs. And the 46 short `let _ = rt.block_on(worker.*)` sites route through `ui::log_worker_err`: a wedged worker can no longer drop queue, playlist, or preference edits without a trace.
- **Fixed:** podcast feed bodies stream through a 32 MiB cap instead of buffering `response.bytes()` unbounded; the feed is untrusted network input arriving on the shared runtime (spec §13), and an over-cap refresh fails with a readable `BodyTooLarge`.
- **Changed:** vir-gtk adopted at 1.4.1 and 1.4.2 (the consumer waves, lock bumps only): the base sheet pins plain `label` text against third-party dark-theme washes (1.4.1), and the final-audit contract fixes land upstream (1.4.2: `close_on_escape` really captures, `Alert` answers once per presentation, `scope_css` splits selector lists at parentheses depth, `at_priority` warns off the scope rung, `bind_settings` stops leaking bound scopes). No Conservatory code changes; `data/cargo-sources.json` regenerated against the moved locks.
- **CI:** `release.yml` answers `v*` tags with the same checks main gets and mints the GitHub Release from the tag's own verbatim message behind an exists-check (the v0.7.0 tag shipped with no Release, so "Latest" advertised v0.6.0; that class dies, and v0.7.0 plus the v0.4.4/v0.4.5/v0.5.0 backfill are live now). The fmt job runs `meson setup` + `meson test`, executing the `appstreamcli --pedantic` and `desktop-file-validate` tests meson.build has wired since 0.4.4; a metainfo or desktop entry that stops validating now fails a push instead of a Flathub submission. `ci.yml` re-syncs to Atrium's hardened shape (CROSS-REPO GRANT #117): permissions floor, concurrency group, SHA-pinned actions, setup-uv v10.1.0 with `enable-cache: false`, and the VERSION sync guard. `dependabot.yml`: github-actions only, weekly (the cargo side stays consumer-wave manual).
- **Docs:** the audit's truth cluster. spec §9's CLI synopsis is rebuilt from the clap definitions (the real argument shapes, flat `shelf-genre-set`, the queue actions that exist, the in-process engine, per-verb `--format`); §7.5's precedence is the code's sidecar > embedded > folder; the ID3-CHAP + `id3` phantom is struck and the unported chapter sources are slotted on the roadmap; symphonia is firmed to considered-and-not-adopted (Phase 1c signed off lofty); the GPL/ffmpeg filter lists are reconciled on the as-built set across spec §15, ATTRIBUTIONS, and libmpv-profiles; the "nightly backup and the JSON export" fiction is reworded to the shipped backup/restore verbs (rotation and the curated export are slotted features); and the scrobble, import, mover, and search-grammar tails are as-built. The version carriers (README badge/status/phase list, spec masthead) name 0.7.0 and the masthead joined the carrier set; "six crates" is the correct five-plus-two-git-deps in the three most-read docs; the milestone table gains its missing 0.4.5 through 0.7.0 rows and 0.2.0 says shipped-never-tagged; the comment truth pass (sixteen sites) and the comment LOW batch land as two commits; the mechanical em-dash recast (172 heading and attribution separators to colons) leaves a recorded prose tail. Housekeeping: the empty-dir markers are gone, the GSettings gitignore fiction is replaced by a `/.flatpak-builder/` ignore, and the repository URL takes the canonical casing. GitHub: SECURITY.md, private vulnerability reporting, and two minimal issue templates.
...
- **Added:** `conservatory-cli backup <db> <out>` snapshots the database through the single-writer worker with `VACUUM INTO`: the statement runs on the writer's own connection inside its blocking task, so the copy is consistent with the WAL and no other write interleaves, and an existing target is refused (a backup is never silently overwritten).
- **Added:** `conservatory-cli restore <db> <backup>` is the documented replace path: the stale `-wal` / `-shm` sidecars of the database being replaced are removed, the backup is copied through a same-directory temp file (fsync, atomic rename, the mover's write_atomic discipline), and any failure before the rename leaves the live file untouched. The CLI then reopens the database so migrations run and the restored file is proven to open, reporting the schema version it reached. Together the two verbs make spec 9's data-safety contract true: the curated layer a re-import cannot rebuild (ratings, play counts, podcast triage, Perspectives) finally has its mechanism. Tests: the snapshot is a restorable library, restore round-trips (post-backup writes gone, the reopened writer accepts writes again), and read-only attaches refuse writes on the live database, the snapshot, and the restored database.
- **Fixed:** move-mode import journals the sibling cover file like the audio (the 2026-09-11 functional-pass finding): a loose `cover.jpg` beside the tracks used to be duplicated into the managed tree and left behind in the otherwise-consumed source folder. The import now resolves where the cover bytes came from; embedded art keeps the canonical-copy path, while a sidecar in move mode rides the journal as a cover-shaped op with a per-source claim. Undo restores both the audio and the sidecar and clears `albums.cover_path` under its guard instead of leaving the album pointing at a moved file; copy mode is unchanged.
- **Added:** the GUI runs startup roll-forward recovery. Only the CLI gated its mutating verbs behind `mover::recover`, so an interrupted job stayed wedged in the GUI until the first move attempt surfaced it; the window now drives recovery right after the pool is stood up and, on failure, shows the CLI's escape-hatch message while browse still comes up.
- **Fixed:** the undo crash window is pinned by tests. The audit's HIGH mover finding (a retry wedging the job with NotFound) does not reproduce: `revert` delegates to `relocate`, whose Phase 2c guard already treats the already-reverted state as a no-op and redoes the move-back when a crash left both copies in place. The integration and unit tests now prove it, and the crash-window contract is spelled out on `revert`.
- **Packaging:** `data/cargo-sources.json` is regenerated from Cargo.lock (the vendored pins had not moved with the consumer waves, so the offline sandboxed build would have failed version resolution exactly as at v0.4.5), and CI gains the Atrium freshness guard: the manifest is regenerated in the fmt job and any drift from the lock fails the run.
- **Docs:** the audit's named docs slice: schema.md gained the 0022 ledger entry and a `track_credits` section and lost the phantom `conservatory-search` name; libmpv-profiles' status header now says the 5.5 chain shipped and replaces the retracted `anequalizer` promise with the named-biquad re-scope; theme.md tells the vir-gtk base_css tier story (shared base at USER + 1, app sheet at USER + 2, accent ring at USER + 3); search-grammar's date-keyword list gains `tomorrow`, `lastweek`, `nextweek`, `lastmonth`, and `nextmonth`; CLAUDE.md's plugin-stub, Meson-timing, and GPL-rationale lines are current; ATTRIBUTIONS gains vir-search and vir-gtk; spec 9's verb line matches the shipped commands.
...
- **Changed:** vir-gtk adopted at 1.4.0 (the widget-kit first-slice wave), riding the gtk4 0.9 to 0.11 bump the 1.3.0 waiver deferred (`v4_14` feature unchanged). Conservatory deletes `dialogs.rs` (the hand-rolled `Alert`/`Appearance`) and the shared subset of `rows.rs` (row builders and `Group`; the `Expander` stays app-side): the kit re-exports the rows under the historical `ui::rows` path, and the Alert/Appearance imports move to `vir_gtk::widgets`. `Alert`'s local surface already matched the kit (Option-taking `present`/`set_extra_child`), so no dialog call sites changed; `close_on_escape` becomes a kit re-export whose capture phase supersedes the local default-phase copy; the six `entry_row` call sites adopt the widened four-`Option` signature. Workspace suite 657 green plus the music-only lane 69; clippy `-D warnings` clean.
...
- **Packaging:** the §5.8 tool-bundling approach is decided: own manifest modules, not a shared extension. The `ffmpeg-full` extension was struck on research (libs-only contract, optional by default, discontinued at the freedesktop 25.08 base the GNOME 50 runtime sits on); the plan is one minimal ffmpeg module yielding `ffmpeg` + `ffprobe`, plus small `flac` and `rsgain` modules (with `libebur128`, `taglib`, `libinih`). Recorded in the Phase 20 box and the manifest notes; module work and verification ride the Phase 20 gate.
- **Added:** track credits (19b-iii, Option A), the richer navigable-credits milestone item. Migration `0022` adds `track_credits (track_id, artist_id, role)` reusing the shared `artists` rows, with roles as TEXT (Composer, Performer, Producer at v1). Import reads the three roles from embedded tags per format; write-back embeds them (Vorbis/MP4 carry all three multi-valued; ID3v2 reliably holds TCOM Composer only, a documented caveat with the database staying canonical); a typed second ID3v2 read also picks up TXXX-spelled credits from other taggers. New `composer:` search field with full SQL push-down, the `credits <db> [query]` CLI verb, and a credits section in the track properties inspector.
- **Added:** Rating and Added browse facets (the Phase 10c follow-ons). The Rating pane shows `Unrated` plus the 1-5 star counts; the Added pane buckets tracks to their added month (`2026-08`), the search grammar's date precision, so the pane reads chronologically. Both filter like any facet and appear at the end of the pane-picker menu; existing `[browse].panes` configs are untouched.
- **Fixed:** ratings imported from DeaDBeeF land on the intended stars. DeaDBeeF writes its POPM byte on a ~stars*63.75 scale, so its 2-, 3-, and 4-star writes (bytes 127, 190, 254) read one star high through the WMP read buckets; those bytes now exact-match to the intended star ahead of the buckets (the Lattice rerate lesson, applied on read). foobar2000/WMP/MusicBee bytes were already correct.
- **Docs:** roadmap record-hygiene burst. The Phase 7b tests box is ticked with a verification note (the filter and state-derivation clauses shipped with their tests; the Perspectives clause was written in error, Perspectives are music-only). The six Phase 25 audit boxes are ticked as folded into 26n's verification tail, with the compact mini-player re-anchored as the phase's one open item at `2.1.0`, and the duplicate 1.0-gate boxes (the 50k memory gate and the full-library move-safety pass, each written under both Phase 15 and Phase 20) are annotated as one-gate-two-boxes so the open count is honest. The floating "deferred and tracked" SQL push-down deferral for the wildcard/list match kinds now really has a box, in a reconciliation addendum.
- **Docs:** the spec masthead, the Phase 19 + 9 section header, and the remaining forward-looking roadmap notes carry the re-anchored milestone label (`0.4.4+`) instead of the consumed `0.4.0` number; `docs/search-grammar.md` documents the wildcard and list match kinds (`foo*`, `*bar`, `(a,b)`) and names the shared `vir-search` crate instead of the absorbed `conservatory-search`.
...
- **Ratings are read at import** (spec §7.1): ID3v2 `POPM` on the foobar2000/WMP byte scale (the generic tag layer drops POPM, so MP3/AAC get a cheap typed second read), Vorbis `RATING` 0–5 integers, and the MP4 0–100 `rate` atom, all bucketed to 0–5 stars. Verified against the real library's `POPM:Windows Media Player 9 Series` frames and lowercase opus `rating` comments. The write-back stance is unchanged: the database owns the rating after import and never embeds it (§5.6).
- **Bulk actions no longer freeze the window.** Deleting, bulk-editing, or re-tagging a large selection, marking a whole podcast Inbox played, or starring a selection used to issue one blocking worker round-trip per row on the GTK thread; a Ctrl+A over a big library could hang the window for seconds. Each is now a single batched worker command in one transaction (`update_tracks`, `set_tracks_genres`, `delete_tracks`, `delete_books`, `set_episodes_played`, `set_episodes_starred`), and the batched deletes renumber queue positions once instead of once per row.
- **Crash-safe config saves.** `config.toml` was written in place, and a crash mid-write would have left a truncated file that (by design) refuses to load, blocking the next launch. Saves now go through the same temp-fsync-rename discipline as the file mover. The waveform cache got the same treatment against two writers racing on one entry.
- **Podcast fetch and download hardening.** A feed host answering 403/404/500 was treated as feed content: parsed, applied to the show, and its (absent) validators overwrote the stored etag/last-modified, poisoning the next conditional GET; non-success statuses now fail the refresh. Extension-less enclosure URLs (`.../play`, tokenized CDNs) gave every episode of a show the identical fallback filename, so two episodes sharing a folder could silently overwrite each other's audio; the fallback now carries the episode id. Concurrent downloads of one episode shared a single `.part` temp file and could interleave; each attempt now writes its own.
- **Saved searches survive round-trips.** Rendering a parsed filter back to text (Perspectives) did not quote regex values or values containing tokenizer boundary characters (`( ) : " ~ ? ! < > =`, `..`), and a free-text `and`/`or`/`not` re-parsed as a stray operator; all of these now re-quote, so what was saved is what re-parses.
- **A latent re-entrancy panic in the facet coalescer is fixed.** The debounce queue's `perform` ran with the state borrow held despite the module documenting re-entrant `add` as supported; a future caller taking up on that would have crashed. The closure now genuinely runs outside the borrow, with a regression test.
- **Design: the flat sheet learns its missing states.** Disabled controls now read as disabled (they were pixel-identical to live ones), the empty-state call-to-action buttons and the footer status bar got their missing rules, sidebar section actions moved into their headers (no more floating icon toolbars adrift in the rail), and a window that opens small and later grows hands the new height to the track list instead of the facet panes.
- The audiobook CLI `--format json` output now escapes titles with the shared JSON escaper instead of Rust's `Debug` (whose braced control-char escapes are not valid JSON), and the audio test fixtures regenerated to carry a rating in each format's native convention.
- Dependency refresh: semver-compatible `cargo update` across the tree (zbus 5.18, zerocopy 0.8.56, and friends), full suite green.
