...
| **`0.3.0`** | **Hyprland-native design (de-adwaita)** | 26 (+ the Phase 25 audits as its verification tail) | in flight |
...
- [ ] **26g (`0.2.6`): podcasts.rs converts.** 4 AlertDialogs (per-show settings gear, subscribe, unsubscribe, delete-downloads), SpinRow ×5 / SwitchRow ×2 / ComboRow ×1 / PreferencesGroup ×2, then drop the file's `adw::prelude` import. Needs `rows::combo_row` (gtk::DropDown suffix; same `set_selected` / `selected` / `connect_selected_notify` surface), added here with its first consumer.
- [ ] **26h (`0.2.7`): audiobooks.rs converts.** 5 AlertDialogs (bulk edit grid, re-shelve confirm, and siblings), SpinRow ×1 / SwitchRow ×2 / PreferencesGroup ×1, drop `adw::prelude`. After this, `adw::` survives only in window.rs (shell + Preferences), sound.rs, and main.rs.
- [ ] **26i (`0.2.8`): the Preferences rebuild (riskiest step).** `open_preferences` + `build_general_page` + `build_library_page` + sound.rs's `add_audio_groups` become a plain modal `gtk::Window` (transient, `close_on_escape`, ~640×560) holding a `GtkStackSwitcher` over a three-page `GtkStack` (General / Library / Sound); groups → `rows::group`, rows → the owned variants plus new `rows::entry_row` (gtk::Entry suffix) and an `Expander` (header row + `gtk::Switch` + `gtk::Revealer` over a nested list) for the DSP compressor / limiter / leveler; config save-on-close moves from `connect_closed` to `connect_close_request`. Containers only: every EQ/DSP handler body, `Adjustment`, and the `apply_dsp` closure stay byte-identical. Acceptance: audible A/B while playing (band drag, compressor toggle, `--debug` shows the af rebuilds), `audio_state` readback from sqlite, reopen-and-verify persistence, backend-switch restart prompt still fires.
- [ ] **26k1 (`0.2.9`): stack and switcher.** `AdwViewStack` → `gtk::Stack` (text-only tabs per §2.4), `AdwViewSwitcher` → header `gtk::StackSwitcher`, `AdwViewSwitcherBar` → bottom box holding a second switcher, `AdwBreakpoint` → a `size_allocate` width watcher (550px threshold, cached crossing flag, pure `is_narrow()` unit-tested, same feature-gating), `adw::Bin` plugin hosts → `gtk::Box`.
- [ ] **26k2 (`0.2.10`): the shell cut.** `adw::HeaderBar` ×2 → `gtk::HeaderBar` with `show-title-buttons = false` (the §2.4 posture), `AdwToolbarView` → a vertical box (header / toast-overlay content vexpand / status bar / Now-bar / switcher bar, preserving the §2.3 stacking rule), imp `ParentType` → `gtk::ApplicationWindow` (drop `AdwApplicationWindowImpl`, `set_content` → `set_child`), `ConservatoryWindow::new` takes `&gtk::Application` (main.rs passes `upcast_ref()`). Rewrite spec §2.3's widget tree in the same commit. Manual matrix: three tabs, the 550px collapse, drawers, no window buttons, GNOME and Hyprland.
- [ ] **26l (`0.2.11`): the owned stylesheet.** New `conservatory/src/theme.rs`: the 21 Dragon hexes as consts, `sheet()` splicing them into a structural template adapted from Colophon's (flat / square / 1px / dense; base rules for buttons, popovers + modelbuttons, tooltips, scrollbars, selection, focus, plus Conservatory's form controls: spinbutton / switch / dropdown / entry / scale / checkbutton, and the class catalog: `.dim-label`, `.title-*`, `.heading`, `.caption`, `.toolbar`, `.navigation-sidebar`, `.boxed-list`, `.suggested-action`, `.destructive-action`, `.flat`, `.linked`, `.osd`, `.circular`, `.background`, `.accent`, `.numeric`, `.pill` deleted, `.toast` squared); `install()` at `STYLE_PROVIDER_PRIORITY_USER + 1`. The main.rs `CSS` const's app rules migrate in (its `@define-color` block deleted, `@`-token consumers retargeted to baked hexes; drop the `\"Adwaita Sans\"` fallback); `ui/accent.rs` registration bumps to USER + 2 so the accent ring keeps outranking the base sheet; `gtk::Settings` prefer-dark line lands. Sheet tests in-module (every hex present; `font-family` count exactly 3; key rules present). Rewrite `docs/theme.md`. This is the visual flip: eyeball every surface on Hyprland and GNOME, including with a themed `~/.config/gtk-4.0/gtk.css` present.
- [ ] **26m (`0.2.12`): the toolkit cut.** Delete `libadwaita` from the workspace and binary Cargo.toml; main.rs: `gtk::Application`, `StyleManager ForceDark` already replaced at 26l; drop `meson.build`'s `libadwaita-1` dependency and CI's `libadwaita-1-dev`; sweep comments and docs (spec masthead + §3.1 + §12 + §13 + §15, `docs/keymap.md` lines naming Adw types, README if it names libadwaita). Gate: `rg 'adw::|adwaita'` clean in `conservatory*/src`, `cargo tree` shows no libadwaita under both feature sets.
- [ ] **26n (`0.3.0` tag): verification tail + release.** The folded Phase 25 audits against the migrated shell (tiling geometry floor at half/third/quarter tiles; small-tile dialog / Now-bar / drawer sizing; mouse-free keyboard walk + shortcuts-vs-keymap sync + a recorded seek-step decision; no-GNOME session run covering keyring degradation, portal FileDialog, logind inhibitor, MPRIS via playerctl and a waybar mpris module; spectrum at 1.25×/1.5× fractional scale; a full GNOME-session feature pass). Memory: manual `VmHWM` comparison against the `v0.2.0` tag build on the same library. A `--no-default-features` *run*, not just build. Author `docs/hyprland.md` (windowrule recipe on the stable app id, Secret Service `exec-once` one-liner, waybar config, portal packages, note that Preferences and dialogs are now real tileable windows). Tick the phase items above, patchnotes milestone entry, VERSION + Cargo.toml to `0.3.0`, tag.
...
- [ ] **The owned stylesheet.** Author the application sheet (flat, square, hard borders, denser spacing; the Dragon palette mapping in `docs/theme.md` retargets from libadwaita named colours to owned classes). Reuse the pilot's generated-sheet machinery where it transfers, adapted to the baked-hex decision.
- [ ] **Packaging and docs follow-through.** Note the Flatpak runtime question for the future Phase 20 manifest (Colophon evaluated GNOME → freedesktop and stayed: GTK4 ships in the GNOME runtime and not in org.freedesktop.Platform); update spec §2.3's widget-tree diagram and `docs/theme.md` to the new posture; `meson.build` and CI drop the libadwaita dependency.
- [ ] **Verification tail.** The audit items folded in from Phase 25 (tiling geometry, small-tile sizing, keyboard coverage, session independence, Wayland integration, CSD posture) run against the migrated shell as this phase's acceptance criteria; `docs/hyprland.md` is authored here; plus a memory pass (manual `VmHWM` comparison against the `v0.2.0` tag on the same library) to confirm neutrality against the spec §13 budgets.
