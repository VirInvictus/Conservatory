...
- [ ] Bulk edit, GTK half (7b-iii-b): multi-select shelf + the bulk-edit dialog (Ctrl+E) + the move preview-and-confirm. A path-affecting edit re-shelves through the same `plan_book_reorg`/`apply_book_reorg`. **(next)**
